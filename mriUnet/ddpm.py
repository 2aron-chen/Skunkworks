...
class DDPM:
    ...
    def __init__(self, diffuser, T=1000, sinusodial=False, L1loss=True, device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        # Define beta schedule
        self.T = T
        if sinusodial:
            self.betas = torch.sin(torch.linspace(0, math.pi/2, T))*0.02+0.0001
        else:
        ...
        self.diffuser = diffuser.to(device) #model
    ...
    def forward_diffusion_sample(self, x_0, t):
        ...
        # mean + variance
        return self.sqrt_alphas_cumprod_t.to(device) * x_0.to(device) + self.sqrt_one_minus_alphas_cumprod_t.to(device) * noise.to(device), noise.to(device)
    ...
    def train(self, trainloader, num_epochs=100):
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in pbar:
                optimizer.zero_grad()
                t = torch.randint(0, self.T, (batch.shape[0],), device=self.device).long()
                loss = self.get_loss(batch, t)
                loss.backward()
                optimizer.step()
                avg_loss += loss.item()
                count += batch.shape[0]
                pbar.set_description(f"Epoch {epoch} | step {step:04d}/{len(trainloader):04d} Loss: {round(avg_loss/count,5)} ")
